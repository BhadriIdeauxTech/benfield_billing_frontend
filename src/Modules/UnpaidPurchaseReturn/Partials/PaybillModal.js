...
const PaybillModal = ({ trigger, SetChecked, purchaseData, handleUnpaid, checkedGST }) => {
  ...
  useEffect(() => {
    form.setFieldsValue({ balance_amount: paymentAmount })
  }, [paymentAmount])
  ...
  const Addproduct = (values) => {
    request.post(`pay/add_purchase_return_creditpay/`, values)
  ...
  const AddprchsunpaidNonGST = (values) => {
    request.post(`pay/add_purchase_return_creditpay_nongst/`, values)
      .then(function (response) {
        console.log(response);
        toast.success('Successfully');
        SetChecked(false)
  ...
  const handlePaymentTypeChange = (value) => {
    ...
    if (value === 'Cheque') {
      ...
      form.setFieldsValue({ balance_amount: formData?.balance_amount })
  ...
  return (
    ...
    >
      <Row gutter={[24, 24]}>
        ...
        <Col span={24} md={12}>
          <InputNumber label={'Bill No'} disabled name={'bill_no'} placeholder={'Bill No'} />
        </Col>

        <Col span={24} md={12}>
          {/* <CustomInputNumber label={'Bill Amount'} disabled precision={2} name={'grand_total'} placeholder={'Bill Amount'} /> */}
          <Form.Item label="Total Amount" name="sub_total">
        ...
        <Col span={24} md={12}>
          <Form.Item label="Balance Amount" name="balance_amount">
            <Input disabled />
          </Form.Item>
        </Col>
