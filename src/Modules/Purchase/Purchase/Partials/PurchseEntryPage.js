...
export const PurchaseEntryPage = ({ editRecord, EditTrigger, closeingModel,FormUpdateClose }) => {
  ...
  useEffect(() => {
    tableData.forEach(record => {
      ...
      form.setFieldsValue({ [`tax_per_value${record.key}`]: record.tax_per_value });
    ...
    if (tableData.length > 1) {
      setTableData(prevState => {
        ...
        newData.forEach(item => {
          if (item.item_qty !== '' || item.cal_amt !== '' || item.dis_amt !== '' || item.tax_per_value !== '') {
            totalQuantity += parseFloat(item.item_qty);
            totalDiscount += parseFloat(item.dis_amt);
            totalTax += parseFloat(item.tax_per_value);
            totalAmount += parseFloat(item.cal_amt);
          }
        });

        // update the grand total value in the tableSecondaryData array
  ...
  const CalculateTotal = (record) => {
    ...
    setTableData(prevState => {
      ...
      item.tax_per_value = record.tax_per_value || 0;
      item.item_qty = record.item_qty;
      item.dis_amt = record.dis_amt || 0;
      // item.tax_per_value = value || 0;
  ...
  const HandlePrice = (value, record) => {
    setTableData(prevState => {
      ...
      // const cal_amt = buy_rate + tax_per_value;
      let CalAmount = 0;
  ...
  const HandleProduct = (value, record) => {
    ...
    setTableData(prevState => {
      ...
      item.tax_percentage = setSelectedProduct.gst_percentage;
    ...
    setTableData(prevState => {
      ...
      if (TaxPercentage != 0) {
        if (DiscountPercentage != 0) {
          ...
          CalculateTotal({
            ...item,
            tax_per_value: item.tax_per_value,
            cal_amt: Amt,
          })
        }
        else {
  ...
  const calculateAmount = (record) => {
    ...
    const buy_rate = parseFloat(record.buy_rate) || 0;
    ...
    let result = {
      ...
      purchase_bill: Object.entries(record)
        ...
        .map(([key, item_name]) => {
          ...
          const taxAmtKey = `tax_per_value${index}`;
  ...
  // console.log(FormDate?.purchase_date,'total ff');
    ...
    <Fragment>
      ...
      >
        ...
        <PurchaseFormHeader
          ...
          trigger={trigger} GetSaleCustomer={GetSaleCustomer}
          setProductList={setProductList} setInvoiceNumber={setInvoiceNumber}
