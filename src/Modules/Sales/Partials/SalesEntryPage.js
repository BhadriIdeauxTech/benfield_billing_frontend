...
import { updatesale } from "../action";



export const SalesEntryPage = ({ setSale, record, ViewEditTrigger, closeingModel, updateTrigger,handleGet }) => {
  ...
  useEffect(() => {
    if (Object.entries(editRecord).length != 0) {
      ...
      const get_invoice_date = new Date(editRecord?.sale?.invoice_date)
      ...
      const invoice_date = dayjs(get_invoice_date).format(dateFormat);
      ...
      setSelectedDate(editRecord?.sale?.invoice_date);
      ...
      // const footData = {
      //   round_off_value: editRecord?.sale?.round_off_value,
      //   grand_total: editRecord?.sale?.grand_total,
      //   received_amt: editRecord?.sale?.received_amt,
      //   balance: editRecord?.sale?.balance,
      // }
  ...
  useEffect(() => {
    if (record) {
      ...
      form.setFieldsValue({ invoice_no: invoiceNumber || 0 })
    }
  }, [editRecord, invoiceNumber, record])
  ...
  const HandleProduct = (value, record) => {
    setTableData(prevState => {
      ...
      HandlePricePer(item.price_percentage, {
        ...item,
        quantity: item.quantity,
        item_cal_total_amt: CalAmount,
        sale_price: item.sale_price,
  ...
  const onFinish = (values) => {
    ...
    let result = {
      ...
      GSTIN: record.GSTIN,
      email: record.email,
      credit_period: record.credit_period || null,
      ...
      sales: tableData?.map((data) => ({
        ...
        tax_cal_amt: parseFloat(data?.tax_cal_amt).toFixed(2),
        tax_percentage: parseFloat(data?.tax_percentage).toFixed(2),
        unit: data?.unit,
      })),
  ...
  const salesEdit = (values) => {
    setIsloading(true);
    request.put(`sales/edit_sale/${record?.id}/`, values)
      .then(function (response) {
        if (response.status == 201) {
          toast.info('Successfully Updated Sales Entry ')
          setIsloading(false);
          form.resetFields();
          setTrigger((trigger) => trigger + 1);
          GetSaleCustomer();
          ...
          setIsloading(false);
          toast.success('Successfully Updated Sales Entry ')
        }
        if(handleGet){
          handleGet();
        }
  ...
  const FooterComponent = () => {
    return (
      <div style={{ background: 'var(--light-color)', paddingBlock: '20px' }}>
        <Flex gap={'10px'}>
  ...
  const onRest = () => {
    ...
    // form.setFieldsValue({customer_name:getdata})    
