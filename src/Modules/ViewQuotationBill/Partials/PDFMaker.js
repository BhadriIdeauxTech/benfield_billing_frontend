            ...
            {Array.from({ length: totalPages }, (_, pageIndex) => (
                <Page key={pageIndex} style={{ padding: '40px 30px' }}>
                    ...
                    {record?.sales
                        ...
                        .map((value, index) => {
                            const formattedAmount = (value?.item_amt_no_gst !== null ? value?.item_amt_no_gst : 0).toLocaleString('en-IN', { minimumFractionDigits: 2 });
                            const formattedTaxRate = (value.price_amount !== null ? value.price_amount : 0).toLocaleString('en-IN', { minimumFractionDigits: 2 });
                            return (
                                <>
                                    <View style={{ display: 'flex', flexDirection: 'row', }}>
