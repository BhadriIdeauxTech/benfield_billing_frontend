...
export const SalesEntryPageGst = ({ record, ViewEditTrigger, FormCloseUpdate }) => {
  ...
  useEffect(() => {
    setEditRecord({})
    if (record) {
      getEditSale(record?.id);
    }
  }, [record, ViewEditTrigger])


  const getEditSale = (id) => {
    request
      .get(`sales/edit_sale_new/${id}/`)
      .then(function (response) {
  ...
  }, [invoiceNumber, editRecord, record])
  ...
  const SalesPost = (values) => {
    setIsloading(true)
    request.post('sales/add_sale_new/', values)
      .then(function (response) {
        if (response.status == 201) {
          toast.success('Successfully Billed Estimate Sales Entry ')
          form.resetFields();
  ...
  const salesEdit = (values) => {
    setIsloading(true)
    request.put(`sales/edit_sale_new/${record?.id}/`, values)
      .then(function (response) {
        if (response.status == 201) {
          toast.success('Successfully Billed Estimate Sales Entry ')
          form.resetFields();
  ...
  const onFinish = (values) => {
    ...
    let result = {
      ...
      customer_name: newvalues.customer_name,
      company_name: newvalues.company_name,
      Address: newvalues.address,
      delivery_location: newvalues.delivery_location,
      mobile_number: newvalues.mobile_number,
      state_of_supply: newvalues.state_of_supply,
      email: newvalues.email,
      credit_period: newvalues.credit_period || null,
      payment_type: newvalues.payment_type,
      reference_no: newvalues.reference_no || null,
      recevied_status: newvalues.recevied_status,
      round_off: newvalues.round_off,
      invoice_date: newvalues.invoice_date,
      old_balance: newvalues.old_balance,
      invoice_no: newvalues.invoice_no,

      delivery_note: newvalues.delivery_note,
      reference_no_new: newvalues.reference_no_new,
      ref_date: refDate,
      other_references: newvalues.other_references,
      buyer_order_no: newvalues.buyer_order_no,
      dated: printDated,
      dispatch_no: newvalues.dispatch_no,
      delivery_note_date: deliveryNoteDate,
      dispatch_through: newvalues.dispatch_through,
      destination: newvalues.destination,
      termsofdelivery: newvalues.termsofdelivery,
      ...
      tax_total: parseFloat(tableSecondaryData[0]?.tax_total).toFixed(2),
      ...
      sales: tableData?.map((data) => ({
        ...
        tax_cal_amt: parseFloat(data?.tax_cal_amt).toFixed(2) || 0,
        tax_percentage: parseFloat(data?.tax_percentage).toFixed(2) || 0,
        // tax_cal_amt: 0,
        // tax_percentage: 0,
        unit: data?.unit,
      })),
  ...
  return (
    <Fragment>
      ...
      >
        ...
        <SalesFormHeader trigger={trigger} setSelectedDate={setSelectedDate} selectedSale={selectedSale} setSelectedSale={setSelectedSale} setInvoiceNumber={setInvoiceNumber} />
        ...
        <Card>
          {record ? <Flex flexEnd gap={'10px'}>
            <Button.Success text={'Update'} htmlType="submit"
              disabled={submitDisable} loading={isloading} />
          </Flex> :
