...
export const PDFMaker = ({ record }) => {
    ...
    const Header = () => {
        return (
            <>
                <Text style={{ textAlign: 'center', marginBottom: '10px', fontSize: '12px', fontWeight: '500' }}>Estimate</Text>
            ...
            {Array.from({ length: totalPages }, (_, pageIndex) => (
                <Page key={pageIndex} style={{ padding: '40px 30px' }}>
                    ...
                    {record?.sales
                        ...
                        .map((value, index) => {
                            const formattedAmount = (value?.item_cal_total_amt).toLocaleString('en-IN', { minimumFractionDigits: 2 });
                            const formattedTaxRate = (value.sale_price).toLocaleString('en-IN', { minimumFractionDigits: 2 });
                            return (
                                <>
                                    <View style={{ display: 'flex', flexDirection: 'row', }}>
