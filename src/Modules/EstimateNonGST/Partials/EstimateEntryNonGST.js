...
import { EstimateFormHeadernongst } from "./EstimateFormHeaderNonGST";
import { EstimateFormFooternongst } from "./EstimateFormFooterNonGST";



export const EstimateEntryPagenongst = ({ editRecord, EditTrigger, closeingModel }) => {
  ...
  const GetSaleCustomer = () => {
    request.get('quotation/get_detail_esti_new/')
  ...
  const QuotationPost = (values) => {
    setIsloading(true)
    request.post('quotation/add_esti_quo_new/', values)
      .then(function (response) {
        if (response.status == 201) {
          toast.success('Successfully Billed Estimate Quotation Entry ')
          form.resetFields();
  ...
  const Edit_Quotation_url = 'quotation/edit_estimate_quotation_new'
  ...
  const QuotationEdit = (values) => {
    ...
    request.put(`${Edit_Quotation_url}/${editRecord?.id}/`, values)
      .then(function (response) {
        if (response.status == 201) {
          toast.success('Successfully Update Estimate Quotation Entry ')
          form.resetFields();
          setTrigger((trigger) => trigger + 1);
          GetSaleCustomer();
          setTableData(initialData);
  ...
  const onFinish = (values) => {
    const record = { ...values, invoice_date: values?.invoice_date === null ? '' : dayjs(selectedDate).format('YYYY-MM-DD') ? dayjs(values?.invoice_date).format('YYYY-MM-DD') : dayjs(values?.invoice_date).format('YYYY-MM-DD'), };
  ...
  return (
    <Fragment>
      <Form name="sales"
        ...
        <EstimateFormHeadernongst
          switchJd={switchJd} setSwitchJd={setSwitchJd}
          setSelectedDate={setSelectedDate} selectedSale={selectedSale}
          setSelectedSale={setSelectedSale} getdataCustomr={getdataCustomr} />
