...
export const ProformaEntryPage = ({ updateRecord, UpdateForm, proformTrigger }) => {
  ...
  const EditGetDetial = 'sales/edit_proforma'

  const EditPostURL = 'sales/edit_proforma'
  ...
  useEffect(() => {
    if (Object.entries(editRecord).length != 0) {
      ...
      if (editRecord?.sale_table) {
        ...
        console.log(editRecord?.sale_table,'editRecord');
...
console.log(item,'item');
  ...
  const ProformPost = (values) => {
    setIsloading(true);
    request.post('sales/add_proforma/', values)
      .then(function (response) {
        if (response.status == 201) {
          toast.success('Successfully Billed Proforma Entry ')
          setIsloading(false);
          form.resetFields();
  ...
  const ProformUpdate = (values) => {
    ...
    request.put(`${EditPostURL}/${updateRecord?.id}/`, values)
      .then(function (response) {
        if (response.status == 201) {
          toast.success('Successfully Update Proforma Entry ')
          setIsloading(false);
          form.resetFields();
          setTrigger((trigger) => trigger + 1);
          GetSaleCustomer();
  ...
  const onFinish = (values) => {
    ...
    // console.log(result, 'resultresultF');
  ...
  return (
    <Fragment>
      <Form name="proforma"
        ...
        <ProformaFormHeader trigger={trigger} setSelectedDate={setSelectedDate} setGetdata={setGetdata} selectedSale={selectedSale} setSelectedSale={setSelectedSale} setInvoiceNumber={setInvoiceNumber} />
        ...
        <div style={{ margin: '20px 0' }}>
          <ProformaFormFooter payType={payType}
