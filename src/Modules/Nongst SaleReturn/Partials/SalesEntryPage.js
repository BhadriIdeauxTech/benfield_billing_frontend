...
export const NonGst_SalesEntryPage = ({ setSaleorder, getSaleorders }) => {
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({ bill_no: selectedSupplierDetails?.invoice_number })
    ...
    let result = {
      ...
      sales: Object.entries(record)
        ...
        .map(([key, item_name]) => {
          ...
          const TaxCalAmt = `tax_cal_amt${index}`;
          const TAXPER = `tax_percentage${index}`
          ...
          return {
            ...
            tax_cal_amt: parseFloat(record[TaxCalAmt]).toFixed(2),
            tax_percentage: parseFloat(record[TAXPER]).toFixed(2),
  ...
  const SalesPost = (values) => {
    request.post('sales/add_salereturn_new/', values)
      .then(function (response) {
        if (response.status == 201) {
          toast.success('Successfully Billed Estimate Sales return Entry ')
          form.resetFields();
  ...
  return (
    <Fragment>
      <Form name="sales"
        ...
        <SalesFormHeader setSelectedDate={setSelectedDate} setProductList={setProductList} Setreset={Setreset} setSelectedSupplierBillDetails={setSelectedSupplierBillDetails} setReturnNumber={setReturnNumber} setSaleorder={setSaleorder} trigger={trigger} setSelectedSupplierDetails={setSelectedSupplierDetails} />
        ...
        <div style={{ margin: '20px 0' }}>
          <SalesFormFooter BalanceOnChange={BalanceOnChange} payType={payType} setPayType={setPayType} RoundOffChecked={RoundOffChecked} HandleCheQueChage={HandleCheQueChage} TotalBalance={TotalBalance} tableSecondaryData={tableSecondaryData} footerCalData={footerCalData} setRoundDecimalValue={setRoundDecimalValue} round={round} />
        </div>
