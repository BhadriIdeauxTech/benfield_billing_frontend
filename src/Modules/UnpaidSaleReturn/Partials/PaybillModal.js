...
const PaybillModal = ({ trigger, SetChecked, suppilerData, checkedGST, handleUnpaid }) => {
  ...
  const Addsaleunpaid = (values) => {
    request.post(`payment/add_sale_return_debit_pay/`, values)
  ...
  const AddsaleunpaidNonGST = (values) => {
    request.post(`payment/add_sale_return_debit_pay_new/`, values)
      .then(function (response) {
        console.log(response);
        toast.success('Successfully');
        SetChecked(false)
    ...
    <div>
      ...
      >
        <Row gutter={[24, 24]}>
          ...
          <Col span={24} md={12}>
            <InputNumber label={'Bill No'} disabled name={'return_no'} placeholder={'Bill No'} />
          </Col>
