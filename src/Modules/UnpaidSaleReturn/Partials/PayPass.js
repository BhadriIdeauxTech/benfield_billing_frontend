...
const PayPassForm = ({ purchaseData,handleUnpaid ,checkedGST}) => {
  ...
  const onFinish = (values) => {
    ...
    if(checkedGST){
      AddsaleunpaidNonGST(result, record.id);
    }
    else{
      Addproduct(result, record.id);
    }
    // Pass the 'id' parameter to the 'Addproduct' function
  ...
  const Addproduct = (values, id) => {
    request.get(`payment/cheque_pass_sale_return/${id}/`, values)
  ...
  const AddsaleunpaidNonGST = (values, id) => {
    request.get(`payment/cheque_pass_sale_return_new/${id}/`, values)
      .then(function (response) {
        console.log(response);
        toast.success('Successfully');
        if (handleUnpaid) {
